/**
 * Professional In-Memory Notification Scheduler
 * 
 * Based on Slack/Discord architecture patterns:
 ...
 * 
 * This replaces the database-polling anti-pattern with modern event-driven architecture.
 */
...
class NotificationScheduler {
  ...
  private eventHandlers: Map<string, (event: NotificationEvent) => void> = new Map();
   ...
   */
  private async loadNotificationItems(userId: string): Promise<ScheduledNotification[]> {
    ...
    try {
      ...
      if (Array.isArray(tasks)) {
        tasks.forEach(task => {
          if (!task.completed && !task.failed && task.task_time && task.task_time !== 'All day') {
            // Only schedule future tasks - FIX: Use proper time comparison
      ...
      if (Array.isArray(events)) {
        events.forEach(event => {
          if (event.event_time && event.event_time !== 'All day') {
            // Only schedule future events - FIX: Use proper time comparison
      ...
      }      this.log(`📊 Fixed: Loaded ${notifications.length} future notification items from ${tasks.length} tasks, ${events.length} events (current time: ${currentTime})`);
   ...
   */
  private scheduleNotificationTimers(notification: ScheduledNotification): void {
    ...
    if (this.config.enable30minReminders && reminder30min > now && !notification.reminder30min) {
      ...
      this.log(`⏰ Scheduled 30min reminder for ${notification.title} at ${reminder30min.toLocaleTimeString()}`);
    }

    // Schedule 5-minute reminder
    if (this.config.enable5minReminders && reminder5min > now && !notification.reminder5min) {
      ...
      this.timers.set(timerId, timer);
      this.log(`⏰ Scheduled 5min reminder for ${notification.title} at ${reminder5min.toLocaleTimeString()}`);
    }

    // Schedule final notification (task overdue / event start)
    if (targetTime > now) {
      const finalType = notification.type === 'task' ? 'task_overdue' : 'event_start';
      ...
      this.timers.set(timerId, timer);
      this.log(`⏰ Scheduled final notification for ${notification.title} at ${targetTime.toLocaleTimeString()}`);
    }
  }
  ...
  private triggerNotificationEvent(event: NotificationEvent): void {
    ...
    if (event.type === 'task_overdue' || event.type === 'event_start') {
      ...
      this.log(`🗑️ Removed completed notification: ${event.notification.title}`);
    }
  }
  ...
  public onTaskCreated(task: any): void {
    if (!this.isInitialized || !task.task_time || task.task_time === 'All day') return;
  ...
  public onTaskDeleted(taskId: number): void {
    ...
    this.cancelNotificationTimers(notificationId);
    this.notifications.delete(notificationId);
    this.log(`🗑️ Deleted task notification: ${taskId}`);
  }

  public onEventCreated(event: any): void {
    if (!this.isInitialized || !event.event_time || event.event_time === 'All day') return;
      const currentTime = getCurrentTimeString();
    if (!this.isTimeInFuture(event.event_time, currentTime)) return; // Skip past events
  ...
  public onEventDeleted(eventId: number): void {
    ...
    this.cancelNotificationTimers(notificationId);
    this.notifications.delete(notificationId);
    this.log(`🗑️ Deleted event notification: ${eventId}`);
  }
  ...
  private cancelNotificationTimers(notificationId: string): void {
    const timerIds = [`${notificationId}-30min`, `${notificationId}-5min`, `${notificationId}-final`];
   ...
   */
  public onNotificationEvent(id: string, handler: (event: NotificationEvent) => void): void {
