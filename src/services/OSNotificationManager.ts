...
class OSNotificationManager {
  ...
  private notificationStateCache: Map<
    string,
    { notified30min?: boolean; notified5min?: boolean }
  > = new Map();
  ...
  public static getInstance(
    config?: Partial<OSNotificationConfig>,
  ...
  public async start(): Promise<void> {
    ...
    this.log(
      "✅ OSNotificationManager: Started successfully (15-second initial delay)",
    );
  }
  /**
  ...
  public stop(): void {
    ...
    //Enhanced cleanup without destroying singleton
  ...
  private async requestPermissions(): Promise<void> {
    try {
      ...
      console.error(
        "OSNotificationManager: Error requesting permissions:",
        error,
      );
    }
  }
  ...
  private async loadActiveItems(): Promise<void> {
    try {
      ...
      this.log(
        `📊 Loading active items for user ${userId} on ${today} at ${currentTime}`,
      );
        ...
        tasks.forEach((task) => {
          if (
            ...
            if (task.task_time > currentTime) {
              ...
              futureTaskCount++;
              this.log(
                `➕ Added FUTURE task: ${task.title} at ${task.task_time}`,
              );
            } else {
              this.log(
                `⏭️ Skipped OVERDUE task: ${task.title} (${task.task_time} < ${currentTime})`,
              );
            }
          }
        });
        ...
        events.forEach((event) => {
          if (event.event_time && event.event_time !== "All day") {
            if (event.event_time > currentTime) {
              ...
              futureEventCount++;
              this.log(
                `➕ Added FUTURE event: ${event.title} at ${event.event_time}`,
              );
            } else {
              this.log(
                `⏭️ Skipped PAST event: ${event.title} (${event.event_time} < ${currentTime})`,
              );
            }
          }
        });
      ...
      this.log(
        `📊 Loaded ${futureTaskCount} tasks and ${futureEventCount} events for monitoring (total: ${this.activeItems.size})`,
      );
    } catch (error) {
      console.error(
        "❌ OSNotificationManager: Critical error loading items:",
        error,
      );
  ...
  private async checkUrgentNotifications(): Promise<void> {
    try {
      ...
      for (const [itemId, item] of this.activeItems.entries()) {
        ...
        const minutesUntilDue = this.calculateMinutesUntilDue(
          item.time,
          currentTime,
        );
        ...
        if (
          ...
          this.log(
            `🚨 Immediate notification sent for overdue item: ${item.title}`,
          );
        }
      }
    } catch (error) {
      console.error(
        "OSNotificationManager: Error in urgent notification check:",
        error,
      );
    }
  }
  ...
  private cleanupExpiredItems(currentTime: string, today: string): void {
    ...
    for (const [itemId, item] of this.activeItems.entries()) {
      ...
      if (minutesUntilDue < -120) {
        // More than 2 hours overdue
        itemsToRemove.push(itemId);
        this.log(
          `🧹 Cleaning up expired item: ${item.title} (${Math.abs(
            minutesUntilDue,
          )} minutes overdue)`,
        );
      }
  ...
  private async processNotifications(): Promise<void> {
    try {
      if (this.isInitialLoad) {
        this.log(
          "🔕 OSNotificationManager: Skipping notifications during initial load",
        );
        return;
      }
      ...
      this.log(
        `🔍 Processing notifications at ${currentTime} (${this.activeItems.size} items)`,
      );
      ...
      for (const [itemId, item] of this.activeItems.entries()) {
        if (item.date !== today) {
          this.log(
            `📅 Skipping item from different date: ${item.title} (${item.date})`,
          );
          continue;
        }
      ...
      this.log(
        `📊 Notification processing complete. Monitoring ${this.activeItems.size} items.`,
      );
    } catch (error) {
      console.error(
        "OSNotificationManager: Error processing notifications:",
        error,
      );
    }
  }
   ...
   */
  private calculateMinutesUntilDue(
    targetTime: string,
    currentTime: string,
  ): number {
    try {
      ...
      if (
        ...
        this.log(
          `⚠️ Invalid time format - target: ${targetTime}, current: ${currentTime}`,
        );
      ...
      if (diff < -720) {
        // More than 12 hours in the past suggests next day
   ...
   */
  private shouldSend30MinReminder(
   ...
   */
  private shouldSend5MinReminder(
    item: NotificationItem,
    minutesUntilDue: number,
  ): boolean {
  ...
  private async send5MinuteReminder(item: NotificationItem): Promise<void> {
    try {
      ...
      const body = `"${item.title}" ${
        item.type === "task" ? "is due" : "starts"
      } in 5 minutes at ${item.time}`;

      await systemNotificationService.showImmediate(title, body, item.type);
      this.log(`🚨 5min reminder sent: ${item.title}`);
    } catch (error) {
      console.error(
        "OSNotificationManager: Error sending 5min reminder:",
        error,
      );
    }
  }
  ...
  private async handleTaskOverdue(item: NotificationItem): Promise<void> {
    try {
      ...
      console.error(
        "OSNotificationManager: Error handling task overdue:",
        error,
      );
    }
  }
  ...
  private async handleEventStart(item: NotificationItem): Promise<void> {
    try {
      ...
      console.error(
        "OSNotificationManager: Error handling event start:",
        error,
      );
    }
  }
  ...
  public async handleTaskCompletion(
    ...
    try {
      ...
      console.error(
        "OSNotificationManager: Error handling task completion:",
        error,
      );
    }
  }
  ...
  private setupEventListeners(): void {
    ...
    const taskCreatedHandler = (event: Event) =>
      this.handleTaskCreated(event as CustomEvent);
    const taskCompletedHandler = (event: Event) =>
      this.handleTaskCompletedEvent(event as CustomEvent);
    const taskDeletedHandler = (event: Event) =>
      this.handleTaskDeleted(event as CustomEvent);
    const eventUpdateHandler = (event: Event) =>
      this.handleEventUpdate(event as CustomEvent);
    const eventCreatedHandler = (event: Event) =>
      this.handleEventCreated(event as CustomEvent);
    const eventDeletedHandler = (event: Event) =>
      this.handleEventDeleted(event as CustomEvent);
  ...
  private removeEventListeners(): void {
    if (this.eventHandlers) {
      window.removeEventListener(
        "task-updated",
        this.eventHandlers["task-updated"],
      );
      window.removeEventListener(
        "task-created",
        this.eventHandlers["task-created"],
      );
      window.removeEventListener(
        "task-completed",
        this.eventHandlers["task-completed"],
      );
      window.removeEventListener(
        "task-deleted",
        this.eventHandlers["task-deleted"],
      );
      window.removeEventListener(
        "event-updated",
        this.eventHandlers["event-updated"],
      );
      window.removeEventListener(
        "event-created",
        this.eventHandlers["event-created"],
      );
      window.removeEventListener(
        "event-deleted",
        this.eventHandlers["event-deleted"],
      );
      window.removeEventListener(
        "retry-mission-refresh",
        this.eventHandlers["retry-mission-refresh"],
      );
    }
  }
  ...
  private async handleTaskUpdate(event: CustomEvent): Promise<void> {
    await this.safeUpdateActiveItems(async () => {
      ...
      if (task.completed || task.failed) {
        ...
        this.log(
          `➖ Removed task from monitoring (completed/failed): ${task.title}`,
        );
        ...
        if (task.task_time > currentTime) {
          ...
          this.log(
            `🔄 Updated task monitoring: ${task.title} (future time: ${task.task_time})`,
          );
        } else {
          // Remove overdue tasks from monitoring
          this.activeItems.delete(itemId);
          this.log(
            `⏭️ Removed OVERDUE task from monitoring: ${task.title} (${task.task_time} < ${currentTime})`,
          );
        }
      } else {
    ...
    this.safeUpdateActiveItems(() => {
      ...
      if (
        ...
        if (task.task_time > currentTime) {
          ...
          this.log(
            `➕ Added FUTURE task to monitoring: ${task.title} at ${task.task_time}`,
          );
        } else {
          this.log(
            `⏭️ Skipped OVERDUE task creation: ${task.title} (${task.task_time} < ${currentTime})`,
          );
        }
      }
    });
    ...
    this.safeUpdateActiveItems(() => {
      ...
      if (eventData.event_time && eventData.event_time !== "All day") {
        ...
        if (eventData.event_time > currentTime) {
          ...
          this.log(
            `🔄 Updated FUTURE event monitoring: ${eventData.title} at ${eventData.event_time}`,
          );
        } else {
          this.activeItems.delete(itemId);
          this.log(
            `⏭️ Removed PAST event from monitoring: ${eventData.title} (${eventData.event_time} < ${currentTime})`,
          );
        }
      } else {
        this.activeItems.delete(itemId);
        this.log(
          `➖ Removed event from monitoring (no time): ${eventData.title}`,
        );
      }
    });
  }
  ...
  private handleEventCreated(event: CustomEvent): void {
    ...
    if (eventData.event_time && eventData.event_time !== "All day") {
      ...
      if (eventData.event_time > currentTime) {
        ...
        this.log(
          `➕ Added FUTURE event to monitoring: ${eventData.title} at ${eventData.event_time}`,
        );
      } else {
        this.log(
          `⏭️ Skipped PAST event creation: ${eventData.title} (${eventData.event_time} < ${currentTime})`,
        );
      }
    }
  }
   ...
   */
  private getNotificationState(itemId: string): {
    notified30min?: boolean;
    notified5min?: boolean;
  } {
  ...
  private updateNotificationState(
    itemId: string,
    state: { notified30min?: boolean; notified5min?: boolean },
  ): void {
   ...
   */
  private async safeUpdateActiveItems(
