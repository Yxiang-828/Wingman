  ...
  checkNotificationPermission().then((granted) => {
    if (granted) {
      ...
      const notification = new Notification(
        `${recurringIndicator}Task Reminder: ${task.title}`,
        {
          body: `Due on ${formatDate(task.task_date)} ${
            task.task_time ? "at " + task.task_time : ""
  ...
  checkNotificationPermission().then((granted) => {
    if (granted) {
      const notification = new Notification(`Event: ${event.title}`, {
        body: `${event.type} at ${event.event_time} on ${formatDate(
          event.event_date,
        )}`,
...
export const checkUpcomingNotifications = (
  tasks: Task[],
  ...
  const todayTasks = tasks.filter(
    (task) => task.task_date === today && !task.completed,
  );
  ...
  todayEvents.forEach((event) => {
    ...
    //  Use time instead of event_time
...
export const checkUpcomingEvents = (eventCache: any, taskCache: any) => {
  ...
  Object.values(eventCache || {})
    ...
    .forEach((event: any) => {
      ...
      const isUpcoming =
        event.date > todayStr &&
        new Date(event.date).getTime() - new Date(todayStr).getTime() <=
          86400000; // Within next 24h

      if (
        (isToday && event.time === getCurrentTime()) ||
