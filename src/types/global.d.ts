...
declare global {
  interface Window {
    electronAPI: {
      ...
      toggleDevTools: () => void; db: {
        ...
        clearChatHistory: (userId: string) => Promise<{ success: boolean }>; getStorageStats: (userId: string) => Promise<{
          tasks: number;
          events: number;
        ...
        getUserSettings: (userId: string) => Promise<{
          ...
          created_at?: string;
          updated_at?: string;
        }>; saveUserSettings: (userId: string, settings: {
          ai_model?: string;
          ai_model_auto_selected?: boolean;
          theme?: string;
          notifications_enabled?: boolean;
        ...
        saveQuickPrompt: (userId: string, promptText: string) => Promise<{ id: number; success: boolean }>;
        deleteQuickPrompt: (promptId: number) => Promise<{ success: boolean }>;
        updateQuickPromptUsage: (promptId: number) => Promise<{ success: boolean }>;

        getDownloadedModels: (userId: string) => Promise<any[]>;
        saveDownloadedModel: (userId: string, modelData: any) => Promise<{ success: boolean; id: number }>;
        deleteDownloadedModel: (userId: string, modelName: string) => Promise<{ success: boolean }>; deleteDiaryEntry: (id: number) => Promise<{ success: boolean }>;
        ...
        deleteRecurringTask: (id: number) => Promise<{ success: boolean; error?: string; deleted?: boolean; changes?: number; deletedTasks?: number }>;
      ...
      notifications: {
        ...
        storeActiveUser: (userId: string) => Promise<{ success: boolean; error?: string }>;
      };
    };
  }
  ...
  namespace Wingman {
    ...
    // Make optional fields ACTUALLY optional with undefined (not null)
    interface Task {
      id: number;
      user_id: string;
      title: string;
