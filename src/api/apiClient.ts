...
export async function apiRequest<T = any>(
  ...
  if (isDataEndpoint) {
    console.warn(
      `Data endpoint ${endpoint} called on apiClient - should use LocalDataManager instead`,
    );
    throw new Error(
      `Data endpoint ${endpoint} should use window.electronAPI.db instead of API calls`,
    );
  }
  ...
  if (!isAuthEndpoint && !Auth.isAuthenticated) {
    return new Promise((resolve, reject) => {
      console.log(
        `Queuing authentication request to ${endpoint} - awaiting user login`,
      );
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (
        ...
        console.log(
          `Backend may be starting, retrying in ${RETRY_DELAY}ms... (${retryCount + 1}/${MAX_RETRIES})`,
        );
      ...
      throw new Error(
        `Authentication API request failed: ${response.status} - ${errorText}`,
      );
    }
    ...
    if (retryCount < MAX_RETRIES) {
      console.log(
        `Authentication request failed, retrying in ${RETRY_DELAY}ms... (${retryCount + 1}/${MAX_RETRIES})`,
      );
      await new Promise((resolve) => setTimeout(resolve, RETRY_DELAY));
      return apiRequest(endpoint, options, retryCount + 1);
    }
