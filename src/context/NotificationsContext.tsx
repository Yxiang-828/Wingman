...
export const NotificationsProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const fetchNotificationsData = useCallback(async () => {
    ...
    try {
      ...
      const pendingTasksFiltered = (tasks || []).filter(
      ...
      console.log(
        `Notifications: Loaded ${pendingTasksFiltered.length} pending tasks, ${todaysEventsFiltered.length} events`,
      );
    } catch (error) {
