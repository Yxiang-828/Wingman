...
// This solves ASAR path issue when building on Windows, happens sometimes when building (runs during npm run electron:build:win)
  ...
  try {
    ...
    try {
      if (fs.existsSync(asarPath)) {
        ...
        console.log(
          `Successfully renamed app.asar to ${path.basename(tmpName)}`,
        );
      ...
      await execAsync(
        `reg add "HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\RunOnce" /v "DeleteAsarFile" /t REG_SZ /d "cmd /c del /F /Q ${asarPath}" /f`,
      );
