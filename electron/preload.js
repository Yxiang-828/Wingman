...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  db: {    // ═══════════════════════════════════════════════════════════════    // TASK OPERATIONS
    ...
    getAllTasks: (userId) =>
      ipcRenderer.invoke('db:getAllTasks', userId),
    saveTask: (task) =>
    ...
    getAllEvents: (userId) =>
      ipcRenderer.invoke('db:getAllEvents', userId),
    saveEvent: (event) =>
    ...
    userNeedsSync: (userId) =>
      ipcRenderer.invoke('db:userNeedsSync', userId),
    markUserSynced: (userId) =>
      ipcRenderer.invoke('db:markUserSynced', userId),
    getCurrentUser: () =>
      ipcRenderer.invoke('db:getCurrentUser'),
    getUserById: (userId) =>
      ipcRenderer.invoke('db:getUserById', userId),
    clearUserCredentials: () =>
      ipcRenderer.invoke('db:clearUserCredentials'),
  ...
  // DIALOG OPERATIONS WITH FOCUS FIX
  dialogs: {
...
// ============================================================================
// This fixes the critical Electron bug where window.confirm() and window.alert()
// break input focus (electron/electron#31917, #41603, #35872)
// Solution: Override native dialogs to call main process with focus restoration
